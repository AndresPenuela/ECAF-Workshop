...
def project_parameters(veg_cover, years):
    ...
    # South of Spain climate assumption: +0.5% erosivity per year
    rain_increase_pct = years * 0.5
    
    # OC dynamics: Equilibrium tied to tolerable soil loss threshold
    baseline_oc = 1.5
    veg_eq = get_equilibrium_veg() 
    
    # Rate of change depends on distance from equilibrium veg cover
    annual_oc_change = (veg_cover - veg_eq) * 0.001
    future_oc = baseline_oc + (annual_oc_change * years)
    
    # Cap OC between heavily degraded (0.5%) and highly rich (5.0%)
...
# Reduced bottom margin because sliders are now in the Streamlit sidebar
...
ax1.axvline(veg_eq, color='gray', linestyle=':', lw=1.5, label='OC Equilibrium Tipping Point')
ax1.axhspan(0, 1.4, color='#2ecc71', alpha=0.15)

ax1.plot([v], [sl], **marker_style)
text1_content = f'{env_context}\n\nSoil Loss: {sl:.2f} t/ha/y\n{status_text}'
ax1.text(0.6, 0.5, text1_content, transform=ax1.transAxes, fontsize=11, fontweight='bold', bbox=box_style, color=text_color)
...
ax2.axvline(veg_eq, color='gray', linestyle=':', lw=1.5, label='OC Equilibrium Tipping Point')

ax2.plot([v], [ro], **marker_style)
text2_content = f'{env_context}\n\nRunoff: {ro:.2f} %'
ax2.text(0.60, 0.50, text2_content, transform=ax2.transAxes, fontsize=11, fontweight='bold', bbox=box_style, color='#2980b9')
